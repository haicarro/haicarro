...
def extract_inspection_audio_files(event):
    query = '''
        SELECT 
        audio.id,
        audio.inventory_id,
        ...
        FROM mart_data_science.inspection_files_engine_audio audio 
        JOIN 
        (
            ...
            FROM mart_data_science.inspection_files_engine_audio s 
    ...
    '''
    schema = (
         'id','inventory_id', 'inspection_id','inspection_report', 'inspection_type'
    )
    audio_source = 'inspection_files_engine_audio'
    new_path = engine_utils.extract_audio_files(event, query, audio_source, schema)
    return new_path

def lambda_handler(event, context):
    start_time = time.time()
    file_name = extract_inspection_audio_files(event)
    end_time = time.time()
